...
let live = null;                      // the one context the module ends up holding

const param = () => ({ value:0, setValueAtTime(){}, exponentialRampToValueAtTime(){} });
class FakeCtx {
  constructor(){ this.state='running'; this.currentTime=0; this.destination={}; live=this; }
...
describe('the mute/volume preference', () => {
  beforeEach(() => { window.AudioContext = FakeCtx; setVolume(0); });
...
describe('cue routing off pop()’s own cls/kind vocabulary', () => {
  beforeEach(() => { window.AudioContext = FakeCtx; setVolume(0.5); unlock(); sfx.plays = 0; });
...
describe('rate limiting and suppression', () => {
  ...
  it('emits an observable rp-sfx event per real cue — the seam the e2e check reads', () => {
    const seen = [];
    const h = e => seen.push(e.detail.name);
