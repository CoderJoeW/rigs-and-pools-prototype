...
// unlocks manufacturing custom parts on-site — see game/fab.ts for the
// design-and-build mechanic itself. Priced and timed well above the top
// SHELLS tier (site-parts.ts): this is meant to read as the single
...
export type FabSlot = 'frame' | 'mobo' | 'cool' | 'psu' | 'unit';
...
export const FAB_MAP: Map<string, Fab> = new Map(FABS.map(p => [p.id, p]));
export const FAB = (id: string): Fab | undefined => FAB_MAP.get(id);
