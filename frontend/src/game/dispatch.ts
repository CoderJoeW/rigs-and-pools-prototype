...
export function installDispatch(G: Game): void {
  /* ---- a site's live output, dispatched cheapest first ---- */
  const srcOut = (site: Site, src: { p: string; n: number }) => {
    const sourcePart = SP(src.p);
  ...
  const siteCapacity = (site: Site) => site.sources.reduce((sum, source) => sum + srcOut(site, source), 0);
  const siteCooling = (site: Site) => site.plants.reduce((sum, plant) => sum + SP(plant.p).cap * plant.n, 0);
  let heatVer = 0;
  ...
  const sitePlantW = (site: Site, extraHeat?: number) => {
    let need = siteHeat(site) + (extraHeat || 0), draw = 0;
    const order = [...site.plants].sort((plantA, plantB) => SP(plantA.p).pue - SP(plantB.p).pue);
    for (const plant of order) {
      if (need <= 0) break;
      const plantPart = SP(plant.p), capacity = plantPart.cap * plant.n, used = Math.min(capacity, need);
  ...
  function sitePlan(site: Site) {
    ...
    for (const src of site.sources) {
      const sourcePart = SP(src.p), out = srcOut(site, src);
  ...
  const easeOf = (chain: ChainState) => { const hash = chainHash(chain); return hash < 1 ? 1 : Math.max(chain.floor, chain.obs) / hash; };
  ...
  function groupAdvice(group: Group) {
    ...
    let best: { chain: ChainState; revPerMh: number } | null = null;
  ...
  const dayDelta = (key: string, now: number): number | null => {
    const was = yday(key);
  ...
  const dayPaceDelta = (key: string, now: number): number | null => {
    const was = yday(key);
