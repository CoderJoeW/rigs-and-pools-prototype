...
import { DESIGN_AXES, MAX_AXIS_POINTS, designTotals, designStats, designCost, type DesignKind } from '../data/customParts.js';
...
export function installFab(G: Game): void {
  ...
  function closeDesign(): void { G.s.design = null; }
  function bumpDesignPick(axisKey: string, delta: number): void {
  ...
  Object.assign(G, { bumpDesignPick, closeDesign, liveTopOf: (kind: DesignKind) => liveTopOf(G, kind), manufacturePart, openDesign });
}
