...
export function installTick(G){
  ...
  function advanceSiteQueues(hrs){
    for(const site of G.s.sites){
      for(let i=site.queue.length-1;i>=0;i--){
        const job=site.queue[i]; job.left-=hrs;
        if(job.left<=0){
          site.queue.splice(i,1);
          let name;
          if(job.kind==='shell'){ site.shell=job.p; name=SITEPART(job.p).name; G.say('site',site.name+' expanded to '+name); }
          else if(job.kind==='source'){ addTo(site.sources,job.p); name=SITEPART(job.p).name; G.say('site',name+' online at '+site.name); }
          else if(job.kind==='storage'){ addTo(site.storage=site.storage||[],job.p);
            name=SITEPART(job.p).name; G.say('site',name+' online at '+site.name); }
          else if(job.kind==='fab'){ site.fab=job.p; name=FAB(job.p).name; G.say('site',site.name+"'s fab is now "+name); }
          else if(job.kind==='mfg'){
            G.s.customParts.push(job.part); PART_MAP.set(job.part.id, job.part);
            name=job.part.name; G.say('site',name+' finished manufacturing at '+site.name);
          }
          else { addTo(site.plants,job.p); name=SITEPART(job.p).name; G.say('site',name+' online at '+site.name); }
  ...
  function driftSiteWindAndBattery(dt, hrs){
    for(const site of G.s.sites){
      ...
      if(G.battKwh(site)>0){
        ...
        site.batt=Math.min(G.battKwh(site), Math.max(0,
          (site.batt||0) + (plan.chW*0.95 + plan.gridChW*0.90 - plan.disW/0.95)*dt/3.6e6));
      }
    }
  }
  ...
  function finishRigBuilds(dt){
    for(const rig of G.s.rigs){
      if(rig.building>0){ rig.building-=dt;
        if(rig.building<=0){ rig.building=0;
          if(rig.rb){ rig.rb=0; G.say('sys',rig.name+' rebuilt and back online');
            G.pop('Rebuild finished',rig.name,'grn',{kind:'build'}); }
          else { G.say('sys',rig.name+' assembled'); G.pop('Build finished',rig.name,'grn',{kind:'build'}); } } }
    }
  }
  ...
  function advanceChains(dt, days){
    for(const chain of G.s.chains){
      ...
      /* anchor0 freezes the save's starting anchor so decay always relaxes
         ...
         persistence's sim-reseed/reset/retune paths overwrite chain.anchor
      ...
      checkMilestones();
      if(G.s.t%(86400*0.75)<dt){ chain.hist.push(G.price(chain)); if(chain.hist.length>110) chain.hist.shift(); }
    }
  }
  ...
  function checkMilestones(){
    ...
    for(const milestone of MILESTONES){
      ...
      let hit=false;
      try{ hit=milestone.check(G.__exports); }
      catch(err){ if(!milestone._warned){ milestone._warned=1;
      ...
      if(hit){
        ...
        let rankIndex=0; for(const [need] of RANKS) if(milestoneCount>=need) rankIndex++;
        const ranked=rankIndex-1>G.s.mile.rank;
        if(ranked){
  ...
  function settleYourPoolBonds(dt){
    for(const pool of G.s.pools){
      ...
      if(pool.scheme==='PPS'){
        ...
        pool.bond-=owed; pool.earned-=owed;
      }
      if(pool.bond<=0){
        pool.bond=0;
  ...
  function wearCardsAndWarnOnHeat(days){
    for(const site of G.s.sites){
      ...
      } else if(site.hotWarn && temp<64) site.hotWarn=false;
      for(const rig of G.siteRigs(site)){
        if(!G.rigLive(rig)) continue;
  ...
  function billPower(dt, days, hrs){
    ...
    for(const site of G.s.sites){
      ...
      const flow=G.flowOf(site), hourCost=G.siteCostPerHour(site)*hrs;
      site.bill[G.band.value==='off'?'off':G.band.value==='peak'?'peak':'sh']+=hourCost;
      if(flow.load>0) site.bill.cool+=hourCost*flow.cool/flow.load;
      const gridRate=site.sources.reduce((minRate,source)=>{const sourcePart=SITEPART(source.p);
        return sourcePart.rate>0?Math.min(minRate,G.rateAt(sourcePart)):minRate;},15.00);
  ...
  function shedAndRestoreOverCapacityRigs(){
    for(const site of G.s.sites){
      ...
      const cutRigs=G.siteRigs(site).filter(rig=>!rig.on&&(rig.cut==='brownout'||rig.cut==='broke')&&rig.building<=0);
      if(cutRigs.length){
        cutRigs.sort((rigA,rigB)=>netIfOn(rigB)-netIfOn(rigA));
        for(const rig of cutRigs){
          if(rig.cut==='broke' && (G.s.cash<20 || netIfOn(rig)<=0)) continue;
          const wasOn=rig.on; rig.on=true;
          const wouldDraw=G.siteDemand(site);
          rig.on=wasOn;
          if(wouldDraw<=(G.siteCapacity(site)+battAvail())*0.97){
            rig.on=true; rig.cut=null;
  ...
  function applyAutoOffPolicy(){
    ...
    for(const rig of G.s.rigs){
      ...
      if(rig.on && netUsd<G.s.offThreshold){ rig.on=false;
        ...
        if(G.siteDemand(site)+G.rigWallW({...rig,on:true})<G.siteCapacity(site)){
          rig.on=true; G.say('sys','Policy: '+rig.name+' back online'); }
      }
    }
  }
  ...
  function fireDueDrips(dt){
    ...
    if(!G.s.dripAt||G.s.dripAt<prevTime-30*86400||G.s.dripAt>prevTime+intervalSeconds) G.s.dripAt=prevTime+intervalSeconds;
    let guard=0;
    while(G.s.t>=G.s.dripAt && guard++<60){ G.fireDrip(); G.s.dripAt+=intervalSeconds; }
  }
  ...
  function samplePoolHashHistory(dt){
    ...
    for(const pool of G.s.pools){
      ...
      (pool.hist=pool.hist||[]).push(G.poolHash(pool));
      if(pool.hist.length>42) pool.hist.shift();
    }
  }

  function addTo(bucket,partId){ const entry=bucket.find(item=>item.p===partId); if(entry) entry.n++; else bucket.push({p:partId,n:1}); }
  function netIfOn(rig){
    const group=G.groupOf(rig);
    const chain=group&&G.chain(group.chain), site=G.site(rig.site); if(!chain||!site) return -999;
    const units=rig.units; if(!units.length) return -999;
    const megahash=units.reduce((sum,unit)=>sum+PART(unit.p).mh*(1-0.4*unit.w),0)*(1+(rig.tune||0));
    const watts=(G.chassisW(rig)+units.reduce((sum,unit)=>sum+PART(unit.p).w*(1+0.5*unit.w),0)*(1+(rig.tune||0)*1.9))/PART(rig.psu).eff;
  ...
  function setGroupChain(group,chainId){
    ...
    if(group.pool!=='solo' && (!currentPool||currentPool.chain!==chainId||!currentPool.live)) group.pool='solo';
  }
  function setGroupPool(group,poolId){ if(poolId!==group.pool){ forfeitGroup(group,'by switching pool'); group.pool=poolId; } }
  function addGroup(){
    const group={ id:G.s.nextGroup++, name:'Group '+G.s.nextGroup, chain:'tessera',
      pool:'solo', pending:0 };
    G.s.groups.push(group); return group;
  }
  ...
  function runBlockWindow(chain, dt){
    ...
    if(networkHash<=0){
      chain.elapsed=0; chain.hadHash=false;
    ...
    if(!chain.hadHash){ chain.hadHash=true; armBlock(chain); }
    chain.elapsed+=dt;
    let guard=0;
  ...
  function drawWinner(chain, networkHash){
    ...
    for(const group of G.s.groups){
      ...
      const groupHashAmount=G.groupHash(group);
      remainingHash-=groupHashAmount; if(remainingHash<=0) return { pool:group.pool, mine:true, group };
    }
    if(G.drawSimWinner) return G.drawSimWinner(chain, remainingHash);
    return { pool:'solo', mine:false };
  }

  function baselineKey(chain, pool){ return pool ? chain.id+'|'+pool.id : chain.id; }
  function blockBaseline(key){
    ...
    const sorted=[...recentValues].sort((valueA,valueB)=>valueA-valueB), mid=Math.floor(sorted.length/2);
  ...
  function awardBlock(chain, winner){
    ...
    if(!pool){
      if(!winner.mine){
        if(winner.sim && G.creditSim) G.creditSim(winner.sim, fullRewardUsd);
        return;
      }
      G.s.blocksSolved++;
      if(Math.random()<chain.orphan*(1-CONN_Q)){ G.s.orphaned++; G.say('bad','Orphaned on '+chain.name); }
      else { G.s.wallet[chain.id]+=fullReward; G.today().earned+=fullReward*G.price(chain); G.today().blocks++;
        ...
        if(jackpot) G.say('jackpot','Jackpot on '+chain.name+' — '+(usd/baseline).toFixed(1)+'x your usual',
          '+'+fmt.c(fullReward),fullReward,chain.tick);
        else G.say('block','Block solved solo on '+chain.name,'+'+fmt.c(fullReward),fullReward,chain.tick);
        if(record){ G.s.bestBlock=usd;
          ...
          'jackpot',{always:true});
        else G.pop('Block solved','+'+fmt.c(fullReward)+' '+chain.tick,'',{kind:'block'});
    ...
    if(pool.scheme==='PPLNS'){
      ...
      if(poolHashTotal>0&&miningHash>0){
        ...
        for(const group of miningGroups){
          ...
          group.pending+=rawShare; const overflow=Math.max(0,group.pending-rawShare);
          group.pending-=overflow; paidOut+=overflow;
        }
        G.s.wallet[chain.id]+=paidOut; G.today().earned+=paidOut*G.price(chain); if(paidOut>0) G.today().blocks++;
        if(share>0.0002){
          ...
          else G.say('pay',pool.name+' found a block','+'+fmt.c(share),share,chain.tick);
          trackBlockUsd(key, usd);
        }
      }
    } else if(winner.mine){
      G.say('pool',(winner.group?winner.group.name:'Your group')+' found a '+chain.name+' block — flat rate, no bonus');
    }
  }
  ...
  function flatDrip(chain, dt){
    for(const group of G.s.groups){
      ...
      const pool=G.poolOf(group.pool); if(!pool||!pool.live||pool.scheme!=='PPS') continue;
      const drip=(dt*G.groupHash(group)/G.diffOf(chain))*chain.reward*(1-pool.fee)*(1-0.02*(1-CONN_Q));
      G.s.wallet[chain.id]+=drip; G.today().earned+=drip*G.price(chain);
    }
