...
import { CARDS, PSUS, PART_MAP, genCardsFor, genPsuFor, type Card, type Psu } from '../data/hardware.js';
...
export function installGenerations(G: Game): void {
  /* ---- generations: regrow the catalogue from the clock ---- */
  const liveCards: Card[] = [...CARDS];
  const livePsus: Psu[] = [...PSUS];
  let builtGen = 0;                       // how far the CATALOGUE is extended — distinct
  function ensureGens(): void {           // from s.gen, which is saved state.
