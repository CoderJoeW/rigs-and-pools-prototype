...
export function installPowerBilling(G: Game): void {
  ...
  function billSitePower(site: Site, dayIdx: number, hrs: number): void {
    ...
    const gridRate = site.sources.reduce((minRate: number, source: { p: string }) => {
      const sourcePart = SITEPART(source.p) as { rate: number };
