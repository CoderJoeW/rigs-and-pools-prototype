import { MILESTONES, RANKS, type Milestone } from '../data/milestones.js';
import type { Game } from './types.js';

export function installMilestoneTracker(G: Game): void {
  ...
  function checkOneMilestone(milestone: Milestone & { _warned?: boolean }): boolean {
    try {
      return milestone.check(G.__exports);
    } catch (err) {
      if (!milestone._warned) {
        milestone._warned = true;
        console.warn('milestone check failed:', milestone.id, err instanceof Error ? err.message : err);
      }
      return false;
    }
  }

  function awardMilestone(milestone: Milestone): void {
