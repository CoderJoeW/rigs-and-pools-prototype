...
const floor=computed(()=>{ const rigs=rigsHere.value, slots=Math.max(g.siteSlots(f.value), rigs.length), cells=[];
  ...
  for(const r of rigs){ if(cells.length>=MAX_TILES) break; const st=g.rigState(r); if(st.dot==='run') running++;
    ...
    cells.push({ key:'r'+r.id, id:r.id, dot:st.dot, n:cells.length+1, chain, hue:chain!=null?CHAIN_HUE[chain]:undefined, size, cards,
...
const designPreview=computed(()=>{ const d=g.s.design; if(!d) return null;
  const site=g.s.sites.find(x=>x.id===d.fid)||f.value, fab=g.FAB(site.fab); const liveTop=g.liveTopOf(d.kind);
...
const axisAtCap=ax=>{ const d=g.s.design; if(!d) return true; const cur=d.picks[ax.key]||0;
  ...
  <div>
    ...
    <div class="card site-hero">
      <div class="site-hero-hd">
        ...
        <span class="site-hero-name">{{ f.name }}</span>
        <button class="btn btn-sm btn-ghost site-hero-rename" @click="startRename">Rename</button>
      </div>
      <div class="site-hero-stats">
        <div class="shs"><div class="k">Hashrate</div>
          <div class="v">{{ fmt.hash(siteHash) }}</div></div>
        <div class="shs"><div class="k">Power</div>
          <div class="v">{{ fmt.w(g.siteDemand(f)) }}</div></div>
        <div class="shs"><div class="k">Temp</div>
          <div class="v" :class="floor.ambient==='hot'?'neg':floor.ambient==='warm'?'amb':''">
            {{ floor.temp.toFixed(0) }}°C</div></div>
      </div>
    </div>
    ...
    <div class="card"><div class="card-bd pt">
      ...
      <template v-else>
        ...
        <button class="btn btn-ghost btn-sm btn-wide" style="margin-top:8px" :disabled="!canDecommission" @click="decommission">
          {{ !canDecommission ? (g.s.sites.length<=1 ? 'Your only site — cannot decommission' : g.siteRigs(f).length>0 ? 'Move its rigs out first' : 'Finish construction first') : decomArm ? 'Tap again to confirm — this cannot be undone' : 'Decommission this site' }}</button>
      </template>
    </div></div>
    ...
    <div class="card">
      ...
      <div v-if="sec.power" class="card-bd">
        ...
        <div class="track-cap" style="margin-top:4px"><span>Bill at this moment</span><b>{{ fmt.usd2(g.siteCostPerHour(f)*24) }}/day</b></div>
        ...
        <button class="btn btn-sm btn-ghost btn-wide" style="margin-top:8px" @click="sec.power=!sec.power">
          {{ sec.power?'Hide':'Show' }} the {{ mix.length }} source{{ mix.length===1?'':'s' }} feeding this</button>
    ...
    <div class="card">
      <button class="rig-hd" style="width:100%" @click="sec.batt=!sec.batt">
        <span style="flex:1;text-align:left"><span class="nm">Battery</span>
          <div class="sb">{{ g.battKwh(f)>0 ? (f.batt||0).toFixed(1)+' of '+g.battKwh(f)+' kWh · '+(plan.chW+plan.gridChW>0?'charging':plan.disW>0?'discharging':'idle') : 'none installed' }}</div></span>
        <span style="font-size:14px">{{ sec.batt?'−':'+' }}</span></button>
      <div v-if="sec.batt" class="card-bd">
        <template v-if="g.battKwh(f)>0">
          <div class="track"><i class="b" :style="{width:Math.min(100,(f.batt||0)/g.battKwh(f)*100)+'%'}"></i></div>
          <div class="track-cap"><span>Stored</span><b>{{ (f.batt||0).toFixed(1) }} kWh · {{ fmt.w(g.battKw(f)) }} rate</b></div>
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-top:7px">
            ...
            <button class="switch" :class="{on:f.gridCharge}" @click="f.gridCharge=!f.gridCharge" aria-label="grid charge" :aria-pressed="!!f.gridCharge"><i></i></button></div>
          <div style="display:flex;align-items:center;justify-content:space-between;margin-top:7px">
            <span style="font-size:13px">Discharge outside peak too</span>
            <button class="switch" :class="{on:f.disAny}" @click="f.disAny=!f.disAny" aria-label="discharge any" :aria-pressed="!!f.disAny"><i></i></button></div>
    ...
    <div class="card">
      <button class="rig-hd" style="width:100%" @click="sec.cool=!sec.cool">
        <span style="flex:1;text-align:left"><span class="nm">Cooling</span>
          <span v-if="(f.temp||0)>=70" class="tag" style="background:var(--red-t);color:var(--red);margin-left:5px">COOKING</span>
          <div class="sb">{{ (f.temp||0).toFixed(0) }}&deg;C · {{ f.plants.length }} unit{{ f.plants.length===1?'':'s' }}</div></span>
        <span style="font-size:14px">{{ sec.cool?'−':'+' }}</span></button>
      <div v-if="sec.cool" class="card-bd">
        <div class="track"><i :class="g.siteHeat(f)>g.siteCooling(f)?'o':g.siteTemp(f)>58?'w':'g'" :style="{width:Math.min(100,g.siteHeat(f)/Math.max(1,g.siteCooling(f))*100)+'%'}"></i></div>
      ...
      <div class="list">
        <div v-for="(j,i) in f.queue" :key="i" class="rowline">
          <span style="flex:1;min-width:0"><span class="nm">{{ g.jobPart(j).name }}</span>
            ...
            <div class="track" style="margin:6px 0 0"><i class="b" :style="{width:((1-j.left/j.total)*100).toFixed(0)+'%'}"></i></div></span>
          <button class="btn btn-sm btn-ghost" :disabled="g.s.cash<g.rushCost(j)" @click="g.rush(f.id,i)">Rush {{ fmt.usd(g.rushCost(j)) }}</button>
        </div>
      </div>
    </div>
      ...
      <div class="sheet-bd">
        ...
        <div v-for="ax in designPreview.axes" :key="ax.key" class="dl">
          ...
          <dd>{{ designPreview.stats[ax.key] }}
            <span class="stepper">
              <button :aria-label="'Decrease '+ax.label" :disabled="!(g.s.design.picks[ax.key]>0)" @click="g.bumpDesignPick(ax.key,-1)">&minus;</button>
...
<style scoped>
.site-hero{padding:12px 14px 14px}
.site-hero-hd{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:12px}
.site-hero-status{display:inline-flex;align-items:center;gap:5px;font-size:9.5px;font-weight:600;letter-spacing:.06em;text-transform:uppercase;color:var(--ink-3)}
.site-hero-status.online{color:var(--green)}
.site-hero-status.hot{color:var(--red)}
.site-hero-status.idle{color:var(--ink-3)}
.site-hero-dot{width:6px;height:6px;border-radius:50%;background:currentColor;flex:none}
.site-hero-status.online .site-hero-dot{box-shadow:0 0 6px color-mix(in srgb,var(--green) 70%,transparent)}
.site-hero-status.hot .site-hero-dot{box-shadow:0 0 6px color-mix(in srgb,var(--red) 70%,transparent)}
.site-hero-name{font-size:18px;font-weight:600;letter-spacing:-.02em;flex:1;min-width:0}
.site-hero-rename{flex:none}
.site-hero-stats{display:grid;grid-template-columns:1fr 1fr 1fr;gap:1px;background:var(--line);border-radius:10px;overflow:hidden}
.site-hero-stats .shs{background:var(--card);padding:10px 12px}
.site-hero-stats .k{font-size:9px;font-weight:600;letter-spacing:.05em;text-transform:uppercase;color:var(--ink-3)}
.site-hero-stats .v{font-family:var(--mono);font-size:15px;font-weight:500;margin-top:2px;letter-spacing:-.02em}
