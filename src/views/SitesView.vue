      ...
      <div class="rigwrap" :class="'ambient-'+floor.ambient">
        <div class="riggrid" :class="{calm:floor.running>20}">
          <template v-for="c in floor.cells" :key="c.key">
            ...
            <div v-else class="rigtile empty" aria-hidden="true">
    ...
    <div class="card">
      ...
      <div v-if="powerOpen" class="card-bd" style="border-top:1px solid var(--line-2)">
        <div class="flow">
          ...
          <div class="flow-bar">
            <i v-for="s in flowIn" :key="s.k" :class="s.k" :style="{width:(s.pct*100)+'%'}"></i>
          </div>
          <div class="flow-lbl">Going to</div>
          <div class="flow-bar">
            <i v-for="s in flowOut" :key="s.k" :class="s.k" :style="{width:(s.pct*100)+'%'}"></i>
          </div>
        </div>
...
.floor-temp{display:inline-block;margin-left:8px;padding:1px 6px;border-radius:999px;font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.02em;background:var(--line-2);color:var(--ink-3);vertical-align:1px}
...
.rigwrap{padding:12px 12px 14px;border-radius:0 0 12px 12px;transition:background-color .6s ease, box-shadow .6s ease}
